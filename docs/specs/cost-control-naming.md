...
| Add package Cost Line flow | Related Cost Component -> Cost Item Account -> Cost Line | Cost lines are added against a CBS Level 2 Cost Component first. The Cost Item Account list is filtered to previously created Cost Item Codes for that component; `+ Add new Cost Item Account` opens a separate pop-out for standard library or custom project-specific accounts. |
