...
| Add package Cost Line flow | Related Control Account -> Related Cost Component -> Cost Item Account -> Cost Line | Cost lines are first filtered by the CBS Level 1 Control Account. The Cost Component list then shows matching CBS Level 2 components, and the Cost Item Account list is filtered to previously created Cost Item Codes for that component. `+ Add new Cost Item Account` opens a separate pop-out for standard library or custom project-specific accounts. |
