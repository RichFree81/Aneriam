...
| `project_tasks` level-2 rows | Deliverables | NetSuite mirror of the CBS L2 tier when surfaced in CBS views. |
| `project_tasks` level-3 rows | Cost Item Codes | NetSuite mirror of the CBS L3 tier when surfaced in CBS views. |
| `package_documents` | Documents / Issue Register | Former `package_deliverables`; renamed so Deliverable means the PBS/CBS L2 asset deliverable. |
...
| `packages.planned_value` | Planned value | Package-level planning reservation value before award. |
