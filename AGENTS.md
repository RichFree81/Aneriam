...
- **Cost Control MVP** — a tactical Windows desktop app for RST internal use, packaged as a single `.exe` via PyInstaller. Lives in `/app`. Independent of the platform; its own SQLite DB, its own templates, its own deployment story. See `app/AGENTS.md` for its rules.
...
└── app/                         # Cost Control MVP — FastAPI + SQLite, packaged as Windows .exe
    ├── AGENTS.md                # Cost-control-specific rules
...
2. Read the sub-AGENTS.md for whichever app you're touching:
   - `frontend/AGENTS.md` and/or `backend/AGENTS.md` for the Aneriam platform.
   - `app/AGENTS.md` for the Cost Control MVP.
