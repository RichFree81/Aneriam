...
- Package cost line forms must start with `Related Cost Component`, then filter the `Cost Item Account` list to accounts already created under that component. If no suitable account exists, the user can select `+ Add new Cost Item Account` at the bottom of the same dropdown.
...
- Package cost line forms must use CBS-aligned labels: `Related Cost Component`, `Cost Item Account`, `Cost line description`, `CBS cost item code`, and `Cost Category (CBS Level 1)`.
