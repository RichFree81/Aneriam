...
- Package cost line forms must start with `Level 2 Cost Component`, then filter the `Cost Item Account (CBS Level 3)` list to accounts already created under that component. If no suitable account exists, the user can select `+ Add new Cost Item Account` and create it from the standard library or as a custom account. Custom accounts are project records only and must not be added back to the standard library.
- Package cost line forms must use CBS-aligned labels: `Level 2 Cost Component`, `Cost Item Account (CBS Level 3)`, `Cost line description`, `CBS cost item code`, and `Cost Category (CBS Level 1)`.
